...
class EncoderRNN(nn.Module) :
    ...
    def forward(self, x):
        ...
        # passes the inputs as tensors to embedding
        tensor_x = torch.tensor(x)
        # embeds the input tensor
        embedded = self.embedding(tensor_x)
        # returns the output and thelast hidden state from rnn
...
class DecoderRNN(nn.Module) :
    ...
    def __init__(self, no_of_output_symbols, embedding_size=16, hidden_size=25, use_attention=True,
        ...
        if use_attention:
            self.output = nn.Linear( hidden_size*2, no_of_output_symbols )
        else: 
    ...
    def forward(self, inp, hidden, encoder_outputs) :
        ...
        # inputs need to be tensors for self.embedding
        embedded = self.embedding(torch.tensor(inp))
        # call rnn and pass the results of it as the output and last hidden state
        rnn_output, hidden = self.rnn(embedded.unsqueeze(1), hidden)
        # if attention is being used
        if self.use_attention:
            # attention mechanism -- defining W, U and v to correct sizes
            W = self.W.expand(encoder_outputs.size(0), -1, -1)
            U = self.U.expand(encoder_outputs.size(0), -1, -1)
            v = self.v.expand(encoder_outputs.size(0), -1, -1)
            # calculate energy using e_ij = v * tanh(W*h_j + U*s_(i-1))
            # permute is to change the order of the dimensions of hidden. bmm is for batch matrix multiplication
            energy = torch.tanh(torch.bmm(encoder_outputs, W) + torch.bmm(hidden.permute(1, 0, 2), U))
            # softmax of energy and v gives the attention
            attention = torch.softmax(torch.bmm(energy, v), dim=1)
            # the context is the batch matrix multiplication of the encoder outputs with the attention
            context = torch.bmm(encoder_outputs.transpose(1, 2), attention).transpose(1, 2)
            # let the output be the concatenation of rnn_output with the context 
            ...
            if self.display_attention:
                ...
                return output, hidden
        # if attention is NOT being used
        else:
